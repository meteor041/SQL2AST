...
RUN_TIME="${RUN_TIME:-${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}_sft_sampling_n8_no_merge}"
EVAL_LOG_DIR="${SFT_EVAL_LOG_DIR:-${REPORT_ROOT}/logs}"
mkdir -p "${EVAL_LOG_DIR}"
EVAL_LOG_FILE="${EVAL_LOG_DIR}/eval_sft_sampling_${RUN_TIME}.log"
exec > >(tee -a "${EVAL_LOG_FILE}") 2>&1

echo "SFT sampling eval log file: ${EVAL_LOG_FILE}"
...
export EVAL_MODEL_PATH="${SFT_OUTPUT}"
export EVAL_WRAPPER_LOG_FILE="${EVAL_LOG_FILE}"
export EVAL_PIPELINE_LOG_FILE="${CSC_SQL_ROOT}/logs/run_pipeline_${RUN_TIME}.log"
export EVAL_WANDB_STAGE_LABEL="sft-sampling"
export EVAL_WANDB_RUN_NAME="${EVAL_WANDB_RUN_NAME:-sql_rm-eval-sft-sampling-${RUN_TIME}}"
